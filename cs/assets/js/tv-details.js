document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchShowDetails = async (showId) => {
        const url = `https://api.themoviedb.org/3/tv/${showId}?api_key=${apiKey}&append_to_response=content_ratings,videos&language=cs-CZ`;
        try {
    ...
    const displayShowDetails = async (show) => {
        ...
        const creditsUrl = `https://api.themoviedb.org/3/tv/${show.id}/credits?api_key=${apiKey}&language=cs-CZ`;
        try {
    ...
    const displaySimilarShows = async (showId) => {
        const url = `https://api.themoviedb.org/3/tv/${showId}/recommendations?api_key=${apiKey}&language=cs-CZ`;
        try {
            const response = await fetch(url);
            const data = await response.json();
