...
async function fetchSuggestions(query) {
    try {
        const url = `https://api.themoviedb.org/3/search/multi?api_key=${apiKey}&query=${encodeURIComponent(query)}&language=cs-CZ`;
    ...
    if (defaultResultsContainer) {
        Promise.all(suggestions.map(async suggestion => {
            ...
            if (mediaType === 'movie' || mediaType === 'tv') {
                ...
                let creditsUrl = `https://api.themoviedb.org/3/${mediaType}/${suggestion.id}/credits?api_key=${apiKey}&language=cs-CZ`;
