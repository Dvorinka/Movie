document.addEventListener('DOMContentLoaded', () => {
  ...
  const fetchUpcoming = async (url) => {
    try {
      ...
      const filteredMovies = data.results.filter(movie => {
        ...
        // Get the current year and next year, but zero out the time portion of the date
        const startOfYear = new Date(`${currentYear}-01-01`);
        const endOfYear = new Date(`${nextYear}-12-31`);

        // Compare the release date with the start and end of this year and the next
  ...
  const fetchMovieDetails = async (movies) => {
    const detailedMovies = await Promise.all(movies.map(async movie => {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${movie.id}?api_key=${apiKey}&append_to_response=release_dates&language=cs-CZ`);
      const detailedMovie = await response.json();
      
      // Get the upcoming release date for the specific country (e.g., US)
      detailedMovie.usReleaseDate = getUpcomingReleaseDate(detailedMovie, 'CZ');
  ...
  const getUpcomingReleaseDate = (movie, countryCode) => {
    const countryRelease = movie.release_dates.results.find(country => country.iso_639_1 === countryCode);
    ...
    if (movie.release_dates && movie.release_dates.results) {
      ...
      if (certification) {
        ...
        switch (certification) {
          case 'NR':
            return { rating: 'NR', hint: 'Neohodnoceno' }; 
          case 'G':
            return { rating: 'G', hint: 'Obecné publikum. Vstup pro všechny věkové kategorie.' };
          case 'PG':
            return { rating: 'PG', hint: 'Doporučuje se vedení rodičů. Některé materiály nemusí být vhodné pro děti.' };
          case 'PG-13':
            return { rating: 'PG-13', hint: 'Důrazně varujeme rodiče. Některé materiály mohou být nevhodné pro děti mladší 13 let.' };
          case 'R':
            return { rating: 'R', hint: 'Vyhrazeno. Osoby mladší 17 let musí mít doprovod rodičů nebo dospělého opatrovníka.' };
          case 'NC-17':
            return { rating: 'NC-17', hint: 'Vstup pro osoby mladší 17 let je zakázán.' };
          case 'U':
            return { rating: 'U', hint: 'Univerzální. Vhodné pro všechny věkové kategorie.' }; // Map 'U' to 'G'
          default:
            return { rating: certification, hint: 'Hodnocení Popis Nedostupné' };
        }
      }
    }
    return { rating: 'NR', hint: 'Neohodnoceno' }; // Default to 'NR' if no certification is found
  };
  ...
  const formatDate = (dateString) => {
    const options = { year: 'numeric', month: 'long', day: 'numeric' }; // Use 'long' for full month names
    return new Date(dateString).toLocaleDateString('cs-CZ', options); // Set locale to Czech
  };
